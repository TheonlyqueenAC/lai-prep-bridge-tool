...
class LAIPrEPDecisionTool:
    ...
    def _generate_clinical_notes(self, profile: PatientProfile, attrition_rate: float) -> List[str]:
        ...
        if attrition_rate >= 0.70:
            notes.append(
                f"⚠️ VERY HIGH attrition risk ({attrition_rate:.0%}). Multiple intensive interventions required.")
        elif attrition_rate >= 0.55:
        ...
        if profile.current_prep_status == "oral_prep":
            ...
            notes.append(
                "💊 Discontinued oral PrEP: LAI-PrEP addresses adherence challenges that led to discontinuation. "
    ...
    def generate_report(self, profile: PatientProfile, assessment: BridgePeriodAssessment) -> str:
        ...
        report.append("")
        report.append(
            f"💡 With recommended interventions: {assessment.estimated_success_with_interventions:.1%} success")
        report.append(
            f"   Potential improvement: +{(assessment.estimated_success_with_interventions - assessment.adjusted_success_rate) * 100:.1f} percentage points")
        report.append("")
